...
## Short excerpt
Boston Children’s / Dana-Farber *Nature* report: prime assembly uses CRISPR dual flaps to integrate DNA up to 12.1 kb in human cells — including G1-arrested cells and primary T cells / HSPCs — without relying on HDR or obligatory double-strand breaks for standard installs.

## Tags / topic labels
`prime assembly` `genome editing` `prime editing` `CRISPR` `large DNA insertion` `HDR` `non-dividing cells` `T cells` `HSPCs` `TRAC` `AAVS1` `IL2RG` `megabase rearrangement` `Nature` `Boston Children’s` `Dana-Farber` `Bauer` `Levesque` `Agarwal` `2026`
...
## Suggested URL slug
`2026-09-20-prime-assembly-gene-sized-dna`
...
## Image caption tip
Prefer schematic of twin pegRNAs → dual 3′ flaps → overlapping donor → integrated cassette at AAVS1/TRAC/IL2RG; optional “G1: HDR ↓ / PA holds” inset. Caption: *“Prime assembly — up to 12.1 kb on target; works without cell division.”* Do not imply in-vivo cures or approved products. Label “preclinical / ex vivo.”

## Differentiation notes for editors
- **vs post-15 (CRISPR islets):** That was a *clinical* hypoimmune donor-islet transplant story. This is a *methods/platform* genome-engineering paper for large DNA payloads — no islet, no immunosuppression claim.
- **vs post-06 (DNA computer):** Wet DNA *computing* for math ≠ genomic *integration* of therapeutic cargo.
- **vs avoid list:** Not Virtual Biotech, BCI, Ryugu, phonons, antimatter, artificial leaf H₂, neutrinos, JWST IC 348, Higgs ZZ, Venus moon, PANXEON, zirconia tandem, HIV bNAbs, CRISPR islets, BlueRock, vacuumtronics NbSe₂, or Bordeaux Fe/Mo UVC ammonia.
